...
-- SELECT * FROM RecursiveDates
  ...
  SELECT d1.DateValue AS "Date",
         ...
         concat(cast(extract(year from d1.DateValue) as char), cast('Q' as char),cast(EXTRACT(quarter FROM d1.DateValue) as char)) AS QuarterID,
         extract(month from d1.DateValue) AS "Month",
         concat(cast(extract(year from d1.DateValue) as char), cast(EXTRACT(month FROM d1.DateValue) as char)) AS MonthID,
